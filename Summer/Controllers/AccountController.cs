    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                string PCode = model.Code;
                IdentityResult result = new IdentityResult();
                // Create admin
                if (role == GenericUserRoles.RolesType.Admin.ToString())
                {
                    if (PCode == StaticCodes.Admin) // confirm with code in db
                    {
                        result = await UserManager.CreateAsync(user, model.Password);
                        await UserManager.AddToRoleAsync(user.Id, GenericUserRoles.RolesType.Admin.ToString());
                    }
                    else
                    { return View(model); }
                }
